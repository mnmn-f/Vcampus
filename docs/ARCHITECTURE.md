...
校园助手通过独立流式 Socket 复用当前登录会话，支持会话历史、取消、归档恢复、MySQL 知识片段检索和 DeepSeek Responses API；图片附件自动选择视觉模型，PDF/DOCX/PPTX/XLS/XLSX 在客户端通过 PDFBox/Apache POI 提取有界文字，附件不持久化。附件加载完成前客户端禁止发送并展示进度；上传后的首次请求失败可复用同一 `requestId` 重试，服务端幂等保存用户消息，客户端直接替换失败回复而不追加重复问题。生成期间会话切换由客户端锁定，历史异步返回还会校验目标会话，避免串入当前对话。

服务端以纯文本提示和流式清洗器约束模型输出，业务 DTO 对日期时间及状态做专用格式化。`ModeIntentClassifier` 先约束三种模式边界：问答中的写指令、代办中的查询指令以及聊天中的 VCampus 查询或操作只返回切换模式提示。未配置 `VCAMPUS_AI_API_KEY` 或 `DEEPSEEK_API_KEY` 时使用本地知识检索和字段投影，不影响服务端启动。工具仅能调用 `AiToolRegistry` 白名单中的现有业务命令；每个工具公开结构化参数说明，模型输出只允许白名单字段。查询事实先由原业务服务返回，`ToolResultFormatter` 再按问题字段收敛；配置模型时只把已授权结果交给模型整理，不允许补造事实。明确写动作不会被查询工具替代，缺参卡会携带原指令和字段标签继续原写工具。所有写操作必须由当前用户二次确认，并由数据库原子领取确认记录以防并发重复执行。管理员路由测试只调用意图解析器，不经过 `ToolBridge`，不会执行真实业务命令；工具状态从调用日志汇总最近调用时间、成功率、平均耗时和最近错误。

知识管理员列表沿用服务端分页；文档导入先在客户端分段预览、修改、去重和勾选，再由单个事务批量提交，任一分段失败则整体回滚。知识测试可记录标准答案和预期命中知识，批量回归复用同一测试边界。学生对单条助手回复提交反馈，管理员按评价、问题类型、时间和处理状态筛选，并可标记待处理、已处理或忽略以及跳转相关知识片段。学生拥有问答入口，AI 知识管理员拥有上述七项管理任务，最终权限仍由业务命令和当前 `SessionContext` 校验。

小松鼠桌宠只位于客户端表现层：`AppFrame` 使用 `SquirrelPetController` 在 `AppShell` 上方增加不占布局空间的分层组件，最小化时切换为独立 `JWindow`。`AiChatPanel` 只通过 `PetActivityListener` 发布无业务数据的状态枚举，桌宠不持有 token、不读取回答正文、不调用网络服务，也不能绕过 AI 工具确认或业务权限。桌宠是否可见复用 `WorkspaceController.canOpen(ModuleId.AI_ASSISTANT)`，退出登录和窗口销毁时随客户端生命周期一并释放。登录页的挥手松鼠由 `LoginBrandPanel` 独立加载，只改变左侧品牌绘制，不接入桌宠状态机，也不改变登录或注册组件。
