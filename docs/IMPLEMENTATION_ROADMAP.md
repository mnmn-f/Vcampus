...
| 1. 身份与学籍 | 注册、登录、登出、职责切换、本人资料/改密、账号管理、角色和会话管理、登录/业务审计、注销申请；学生档案、成绩、教师登记和学籍管理员核对 | 已完成。注销批准在同一事务禁用账号，提交后才清理运行时会话 |
...
| 3. 图书馆与商店 | 图书、借还、自习室、线上资源访问与日志、图书管理员借阅台账和 CSV 导出、图书馆公告；商品、购物车、订单、账户、支付、取消/状态处理、销售统计 | 已完成。借阅台账由 BORROW_ADMIN_LIST 分页筛选，CSV 单次最多 5000 条且覆盖已有文件需确认；线上资源访问写入日志并由管理员分页查看。销售统计由 MySqlStoreSalesRepository 直接聚合已支付 PAID/COMPLETED 订单；V1 视图只作兼容辅助 |
...
2. **数据库门禁**：新库按 [V1__baseline.sql](../vcampus-server/src/main/resources/db/migration/V1__baseline.sql) → [V2__demo_data.sql](../vcampus-server/src/main/resources/db/migration/V2__demo_data.sql) 执行；重复执行 V2 不新增重复演示数据。检查结果以 [DB_COMPATIBILITY_REPORT.md](DB_COMPATIBILITY_REPORT.md) 为准。
