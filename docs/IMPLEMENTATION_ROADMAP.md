...
| 6. AI 助手与桌宠 | 问答/聊天/代办、多轮会话、DeepSeek、附件、混合知识检索、校规知识、结构化工具澄清；透明小松鼠桌宠、喂食抚摸、最小化收束和 AI 状态联动 | 已完成。20 个写工具继续由原业务模块确认和校验；桌宠互动只属于客户端表现层 |
...
2. **数据库门禁**：新库按 V1 → V2 → [V3__academic_insights.sql](../vcampus-server/src/main/resources/db/migration/V3__academic_insights.sql) → [V4__store_experience.sql](../vcampus-server/src/main/resources/db/migration/V4__store_experience.sql) → V5 → [V6__teacher_time_preferences.sql](../vcampus-server/src/main/resources/db/migration/V6__teacher_time_preferences.sql) → [V10__ai_assistant_knowledge.sql](../vcampus-server/src/main/resources/db/migration/V10__ai_assistant_knowledge.sql) → [V11__ai_knowledge_and_tools.sql](../vcampus-server/src/main/resources/db/migration/V11__ai_knowledge_and_tools.sql) → [V12__ai_quality_workbench.sql](../vcampus-server/src/main/resources/db/migration/V12__ai_quality_workbench.sql) 执行；检查结果以 [DB_COMPATIBILITY_REPORT.md](DB_COMPATIBILITY_REPORT.md) 为准。
...
- AI 已接入 DeepSeek Responses API、MySQL 知识检索、经审核政策摘要、聊天附件和白名单业务工具；外部知识库同步、文档自动导入、独立向量数据库及无需确认的自动写操作未接入。
