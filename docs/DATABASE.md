...
AI 对话先写入会话和消息表，知识问答只检索 `ai_knowledge_chunks.status = 'ACTIVE'` 的片段；同一 requestId 的重试在已有用户消息时不重复插入。知识文档的选中分段在一个事务中去重、保存并生成版本，任一分段失败时整体回滚。`ai_answer_feedback` 只保存脱敏问题摘要；V16 增加 `PENDING`、`RESOLVED`、`IGNORED` 处理状态、可空关联知识、处理人和处理时间。当前未部署独立向量库：服务端从 MySQL 读取有界候选并在内存做关键词、同义词和轻量文本向量混排。写工具先在 `ai_tool_call_logs` 创建具有有效期的待确认记录；参数不完整时先澄清，不创建待确认记录。确认时由数据库原子领取，随后携带当前登录会话重新进入原 `CommandRouter`。工具状态统计从调用日志聚合调用次数、成功率、平均耗时、最近调用和最近错误。AI 不直接写其他业务表，也不接受问题文本指定他人身份。聊天附件不写入消息表；小松鼠桌宠只在本机 `Preferences` 中保存位置和好感度。
