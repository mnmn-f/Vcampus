...
- AI 在当前版本只保留 `ai_chat_sessions`、`ai_chat_messages`、`ai_knowledge_chunks` 和 `ai_tool_call_logs` 四张存储表；不实现模型、RAG 检索或实际写操作工具。
...
AI 相关表当前只是协议与存储预留，没有真实模型、RAG、知识维护服务或工具调用服务。未实现能力不向任何角色显示入口，也不能把表结构或占位文本写成已交付功能。
