...
8. 配置模型 API 时，仅把已授权且已按字段裁剪的实时结果交给模型整理；模型不可用或返回空文本时使用本地结果。提示词约束不等同于可证明的事实校验，外部模型仍需单独验收；
...
- 代办模式同时支持校园查询、知识问答和操作；查询直接回答，不再强制切换，写操作仍必须确认；
...
1. 流式读取 ACTIVE 知识，每批最多 500 条参与排序，保留当前最佳结果后继续，避免旧制度被新增资料挤出候选范围；
2. 对问题进行校园同义词扩展；
3. 计算标题/正文关键词分值；
4. 计算字符词项向量的余弦相似度；
5. 过滤仅有无意义单字重合的候选；
...
项目根目录运行 scripts/test-ai.ps1 可执行全模块回归。脚本只统计本轮生成的 Surefire 报告，构建或断言失败返回非零退出码；跳过的数据库测试不视为通过。真实 MySQL 测试需显式启用 vcampus.mysql.integration=true，且必须使用隔离验收库。最新结果及未完成联调见第 16 节。
...
## 16. 2026-09-14 修复与验收清单

本轮以更新后的项目及《改进建议 - 副本.docx》为依据，未修改登录框、注册流程、桌宠设置，也未覆盖用户已有的演示数据与测试文件变更。

### 16.1 学生端改进

| 问题 / 示例 | 当前处理方式 |
|---|---|
| “退选课程，编号是TEST-C-016” | 课程代码作为名称匹配，不取字符串里的 16；退选只从本人 ENROLLED 记录中解析课程数据库编号，不混用选课记录编号 |
| “帮我借《扩展测试图书35》” | 完整书名精确匹配；35 不再当作图书数据库编号，同名时要求选择 |
| “我想知道扩展测试图书35的信息” / “……的作者是谁” | 清理请求前后缀，保留完整实体；仅作者问题先裁剪为书名和作者，详情问题保留详细字段 |
| “我没选什么课” | 读取课程分页并与本人选课记录比较；退选记录不排除课程，已完成课程不作为未选课；列表不保证容量和时间冲突一定满足 |
| 退课后重选 | 不再把课程名称数字定位到其他课程；业务冲突判断限定同一学期，同时保留重复选课、容量、日期与节次检查 |
| “我预约了什么自习室” / “查询本人请假、报修” | 识别为本人记录查询，而不是新建操作 |
| “查询可申请教室” | 查询教室列表，不误触发教室申请 |
| 自习室、教室预约补参 | 下拉选项来自当前会话授权的业务列表，显示名称、编号和可用的状态/时间信息；日期仍使用日历输入 |
| 取消自习室、教室、请假 | 从本人可操作记录选择；取消不再索要新预约的用途和起止时间 |
| 请假类型反复补充 | 中文类型映射为合法枚举；卡片使用下拉框，重复字段以最后一次输入为准；错误枚举或起止时间返回可修正的卡片 |
| 点击旧参数卡 | 当前卡片绑定自己的原请求，不误用最近一条聊天；历史重新加载的卡片不直接执行；提交按钮防止重复点击 |
| 未支付订单 | 根据 CREATED 等业务状态筛选，不混入 PAID；金额不再被误作分页记录数 |
| 购物车下单、修改数量 | 支持自然语言“下单”；数量单独提取，不再受商品名称或编号中的数字干扰；修改/移除从当前购物车定位商品 |
| 水电相关公告正文 | 公告意图优先于水电账单意图；详情和正文可以展开对应字段 |
| 取消成功但返回空数据 | 写命令成功且返回 null 时明确说明操作成功、业务记录已更新；异常不伪装成成功 |
| 我的课表 | 表格显示课程、学期、星期、节次、教室、日期范围；支持列排序、横向滚动和 UTF-8 CSV 导出，覆盖文件前确认 |
| 校纪校规总览 | 提高真实制度材料优先级，降低“知识回答范围”等元信息权重；按已有材料概括，不因缺少总览文件就否认已收录条款 |
| 旧知识被遗漏 | 不再只读取最新 500 条；MySQL 流式分批排序保留最佳命中 |

确定性解析负责常见意图和结构化补参；模型负责未命中表达的白名单工具分析、知识回答以及投影后结果的语言整理。API 不是数据库事实来源，不能绕过对象验证、当前会话权限或用户确认。接口未配置时，确定性工具路径与本地格式化仍可使用。

业务选择列表最多显示 100 项，分页读取安全上限为 20 页；超出读取上限会提示缩小条件，不会默默选择第一条。业务返回文本有长度上限；大量结果应在原业务页查看。实体自动匹配采用完整名称或业务代码匹配，近似拼写和同名对象不会直接触发写入。

### 16.2 管理员工作流

1. 在“用户反馈”筛选待处理问题并查看详情，关联需要修正的知识。快速重复筛选时，较旧请求不再覆盖新筛选结果。
2. 在“知识库管理”修订条目或导入材料；检查分段及来源，保留版本审计，必要时回滚。
3. 在“知识测试 / 批量回归”核对答案关键词和预期知识，避免只看模型回答流畅程度。
4. 在“路由测试”使用“全工具回归（47 项）”，查看每题的预期工具、实际工具、参数及缺参原因；支持复制诊断报告。运行中禁用重复发起，异常按项记录。
5. 在“工具状态 / 运行监控”核对模块是否注册、失败次数、耗时和最近错误。路由通过不代表数据库一定允许执行，真实业务状态仍由原模块判断。
6. 复现、修订并复测通过后，再将反馈改为已处理。管理员诊断不会自动选课、下单、支付或操作学生账号。

### 16.3 自动化与验证边界

2026-09-14 最终执行 Maven package 成功：共 489 项测试，0 失败、0 错误、12 项环境依赖测试跳过（实际执行通过 477 项）。客户端和服务端 JAR 已重新生成；参数卡与课表的离屏预览已人工检查。此统计只对应本轮代码，不作为未来版本的固定测试数量。
...
新增自动化覆盖全部 47 个工具的示例路由和 DTO/业务命令适配，以及名称含数字、课程代码、同名歧义、非法对象编号、重复补参、请假枚举、时间顺序、取消记录选择、未支付订单、知识候选超过 500 条、真实制度排序、课表/卡片控件与离屏绘制。工具适配测试使用隔离命令处理器，不能替代真实业务表端到端测试。

离屏卡片预览在客户端测试后生成于 vcampus-client/target/ai-card-preview.png；可运行包为 vcampus-client/target/vCampusClient.jar 和 vcampus-server/target/vCampusServer.jar。部署时客户端和服务端应一起更新并重启。

当前执行环境未设置 AI API 密钥与 MySQL 连接变量。因此，本轮未宣称真实 API、真实数据库写入或所有自然语言表达都已验证。现有 MySQL 集成用例默认跳过；只应在隔离测试库中启用，具体步骤见 DB_COMPATIBILITY_REPORT.md。需要完成以下人工联调后才能签署全功能验收：

- 学生 A / B：本人查询、他人编号拒绝、学籍与成绩字段边界。
- 教务：选课→已选列表/课表→退课→重选；同学期冲突应拒绝、跨学期不误报；未选列表与业务页面一致。
- 图书馆：含数字及重名图书借还；预约补参→确认→业务页核对→取消；开放时间、占用冲突和过期记录由业务层拒绝。
- 商店：加入/修改/移除购物车→下单→未支付列表→确认支付→余额与订单状态核对；库存不足、余额不足、重复确认不重复扣款。
- 宿舍与校园：报修、请假、取消请假、水电支付、教室申请/取消、竞赛报名/取消；本人记录与业务页面一致。
- 管理员：知识新增/修改/停用/版本回滚/批量导入事务、反馈关联与处理、47 项路由回归、工具失败指标的变化。
- 模型：真实 API 成功、超时、无密钥和中途失败；检索无证据不编造制度，作者/成绩等字段不扩展为无关信息；聊天附件、停止生成、重试与历史会话恢复。
- 安全：未确认不执行、取消不执行、确认过期拒绝、重复/并发确认仅执行一次，角色切换后重新按当前权限检查。
