...
public final class PdfWorkflowTest {
    ...
    @Test public void databaseFailureCompensatesSealedUploadFile() throws Exception {
        Path root = temporary.newFolder("compensation").toPath();
        PdfFileStore files = new PdfFileStore(root);
        PdfLibraryService service = new PdfLibraryService(new FailingPdfRepository(), files, null);
        Path source = temporary.newFile().toPath(); byte[] bytes = pdf(128); Files.write(source, bytes);
        String sha = PdfFileStore.sha256(source);
        String id = files.begin(a.getUserId(), new PdfUploadRequest("失败测试", "", "x.pdf", bytes.length, sha));
        files.append(a.getUserId(), new PdfUploadChunk(id, 0, bytes));
        try { service.commitUpload(a, id); org.junit.Assert.fail("应报告元数据保存失败"); }
        catch (RuntimeException expected) { /* 文件补偿由服务完成。 */ }
        org.junit.Assert.assertFalse(Files.exists(files.file(sha)));
    }

    private static final class FailingPdfRepository implements PdfRepository {
        @Override public PageResult<PdfResourceView> list(Connection c, PdfQuery q, long owner) { return null; }
        @Override public PdfResourceView find(Connection c, long id, boolean lock) { return null; }
        @Override public PdfResourceView save(Connection c, PdfResourceView value) throws Exception { throw new java.sql.SQLException("metadata failure"); }
        @Override public PdfDownloadRecord download(Connection c, long id, boolean lock) { return null; }
        @Override public PdfDownloadRecord saveDownload(Connection c, PdfDownloadRecord value) { return null; }
        @Override public PageResult<PdfDownloadRecord> history(Connection c, long owner, PdfQuery q) { return null; }
    }
}
