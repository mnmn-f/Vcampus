...
public final class ToolIntentParserTest {
    ...
    @Test public void writesExtractIdAndRemainConfirmable() {
        ...
        assertEquals("library.book.return",
                parser.parse("帮我归还图书 软件工程实践导论").getToolName());
        assertEquals("campus.competition.cancel",
                parser.parse("取消报名校园创新实践演示赛").getToolName());
    }
    ...
    @Test public void genericLibraryAndStoreQuestionsDoNotBecomePunctuationKeywords() {
        assertEquals("{\"keyword\":\"\"}",
                parser.parse("图书馆里有什么书？").getArgumentsJson());
        assertEquals("{\"keyword\":\"\"}",
