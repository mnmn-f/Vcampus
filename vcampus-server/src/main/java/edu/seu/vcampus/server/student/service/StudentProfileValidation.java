...
final class StudentProfileValidation {
    ...
    private static void formats(StudentProfileWriteRequest request) throws StudentRecordException {
        try {
            edu.seu.vcampus.common.validation.StudentCohortYears.validate(request.getClassName(),
                    request.getEnrollmentYear(), request.getExpectedGraduationYear());
            InputRules.studentNumber(request.getStudentNo());
            if (request.getEmergencyContact() != null) {
                InputRules.personName(request.getEmergencyContact(), "紧急联系人");
            }
            InputRules.contactPhone(request.getEmergencyPhone(), false);
