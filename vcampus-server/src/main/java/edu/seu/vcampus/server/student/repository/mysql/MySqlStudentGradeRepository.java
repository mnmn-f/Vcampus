...
final class MySqlStudentGradeRepository
    ...
    private static final String GRADE_COLUMNS =
            "cg.id AS grade_id, e.id AS enrollment_id, e.student_user_id, "
                    ...
                    + "cg.recorded_by, cg.recorded_at, cg.remark, e.status AS enrollment_status";
    private static final String GRADE_FROM =
    ...
    public StudentGradePage review(Connection connection, StudentGradeReviewQuery query) {
        return page(connection, query.getStudentUserId(), query.getCourseId(),
                query.getPage(), query.getPageSize(), query.getOffset());
    }

    @Override
    public List<StudentGradeDto> findAll(Connection connection, long studentUserId) {
        String sql = GRADE_FROM + " WHERE e.student_user_id = ? AND e.status <> 'DROPPED'"
                + " ORDER BY c.course_code";
        List<StudentGradeDto> result = new ArrayList<StudentGradeDto>();
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setLong(1, studentUserId);
            readRows(statement, result);
            return result;
        } catch (SQLException ex) {
            throw failure("failed to query all student grades", ex);
        }
    }
    ...
    public void upsert(Connection connection, long recorderUserId,
        ...
        try (PreparedStatement statement = connection.prepareStatement(INSERT_GRADE)) {
            ...
            if (request.getGradePoint() == null) statement.setNull(3, Types.DECIMAL);
            else statement.setBigDecimal(3, request.getGradePoint());
    ...
    private StudentGradePage page(Connection connection, Long studentUserId, Long courseId,
        ...
        String where = gradeWhere(studentUserId, courseId);
        ...
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            int index = bindFilters(statement, studentUserId, courseId, 1);
    ...
    private long count(Connection connection, Long studentUserId, Long courseId) {
        ...
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            bindFilters(statement, studentUserId, courseId, 1);
            try (ResultSet result = statement.executeQuery()) {
                result.next();
    ...
    private static int bindFilters(PreparedStatement statement, Long studentUserId,
                                   Long courseId, int index) throws SQLException {
        if (studentUserId != null) statement.setLong(index++, studentUserId);
        if (courseId != null) statement.setLong(index++, courseId);
        return index;
    }

    private static void readRows(PreparedStatement statement, List<StudentGradeDto> output)
            throws SQLException {
        try (ResultSet result = statement.executeQuery()) {
    ...
    private static StudentGradeDto readGrade(ResultSet result) throws SQLException {
        ...
        return new StudentGradeDto(result.getLong("grade_id"),
                ...
                result.getString("course_name"), result.getBigDecimal("score"),
                result.getBigDecimal("grade_point"), result.getLong("recorded_by"),
