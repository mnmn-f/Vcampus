...
public final class AiFeedbackService {
    ...
    public List<AiFeedbackEntry> latest() {
        return latest(new AiFeedbackQuery(null, null, null, null, null));
    }
    public List<AiFeedbackEntry> latest(final AiFeedbackQuery query) {
        return tx(new Work<List<AiFeedbackEntry>>() { public List<AiFeedbackEntry> run(Connection c)
                throws Exception { return repository.latest(c, query, 200); }});
    }
    public void update(final SessionContext session, final AiFeedbackTriageRequest request) {
        if (request == null || request.getFeedbackId() <= 0
                || !("PENDING".equals(request.getProcessStatus())
                || "RESOLVED".equals(request.getProcessStatus())
                || "IGNORED".equals(request.getProcessStatus()))) {
            throw new AiServiceException(ResultCodes.INVALID_INPUT, "反馈处理参数不正确");
        }
        Boolean updated = tx(new Work<Boolean>() { public Boolean run(Connection c) throws Exception {
            if (request.getRelatedChunkId() != null && !repository.knowledgeExists(c,
                    request.getRelatedChunkId().longValue())) {
                throw new AiServiceException(ResultCodes.INVALID_INPUT, "关联知识片段不存在");
            }
            return Boolean.valueOf(repository.update(c, session.getUserId(), request));
        }});
        if (!updated.booleanValue()) throw new AiServiceException(ResultCodes.NOT_FOUND, "反馈不存在");
    }
