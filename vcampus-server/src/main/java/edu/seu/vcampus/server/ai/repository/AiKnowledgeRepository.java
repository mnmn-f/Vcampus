...
public final class AiKnowledgeRepository {
    ...
    public AiPage<AiKnowledgeChunk> search(Connection c, AiKnowledgeQuery q) throws Exception {
        ...
        Long chunkId = q == null ? null : q.getChunkId();
        String where = " WHERE (? IS NULL OR id=?) AND (? IS NULL OR status=?) "
                + "AND (? IS NULL OR title LIKE ? OR content LIKE ?)";
        long total = count(c, where, chunkId, status, keyword);
        ...
        try {
            bindFilter(ps, chunkId, status, keyword); ps.setInt(8, size); ps.setInt(9, (page - 1) * size);
    ...
    public boolean duplicate(Connection c, String content) throws Exception {
        PreparedStatement ps = c.prepareStatement(
                "SELECT 1 FROM ai_knowledge_chunks WHERE content=? LIMIT 1");
        try {
    ...
    private long count(Connection c, String where, Long chunkId, String status, String keyword) throws Exception {
        PreparedStatement ps = c.prepareStatement("SELECT COUNT(*) FROM ai_knowledge_chunks" + where);
        try { bindFilter(ps, chunkId, status, keyword); ResultSet rs = ps.executeQuery();
            try { rs.next(); return rs.getLong(1); } finally { rs.close(); } }
        finally { ps.close(); }
    }

    private void bindFilter(PreparedStatement ps, Long chunkId, String status, String keyword) throws Exception {
        String like = keyword == null ? null : "%" + keyword + "%";
        if (chunkId == null) { ps.setNull(1, java.sql.Types.BIGINT); ps.setNull(2, java.sql.Types.BIGINT); }
        else { ps.setLong(1, chunkId.longValue()); ps.setLong(2, chunkId.longValue()); }
        ps.setString(3, status); ps.setString(4, status); ps.setString(5, keyword);
        ps.setString(6, like); ps.setString(7, like);
    }
