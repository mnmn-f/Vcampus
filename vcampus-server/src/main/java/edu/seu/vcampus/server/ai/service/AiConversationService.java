...
public final class AiConversationService {
    ...
    public List<AiSessionSummary> archived(final SessionContext session) {
        return tx(new Work<List<AiSessionSummary>>() {
            public List<AiSessionSummary> run(Connection c) throws Exception {
                return repository.archived(c, session.getUserId());
            }
        });
    }

    public void restore(final SessionContext session, final String id) {
        tx(new Work<Void>() {
            public Void run(Connection c) throws Exception {
                if (!repository.restore(c, id, session.getUserId())) {
                    throw new AiServiceException(ResultCodes.NOT_FOUND, "归档会话不存在");
                }
                return null;
            }
        });
    ...
    public String beginQuery(final SessionContext session, String requested,
        ...
        tx(new Work<Void>() {
            public Void run(Connection c) throws Exception {
                ...
                if (!repository.hasUserRequest(c, sessionId, requestId)) {
                    repository.append(c, sessionId, requestId, "USER", text, "COMPLETED");
                    repository.titleFromFirstQuery(c, sessionId, text);
                }
                return null;
            }
        });
