...
public final class AiToolRepository {
    ...
    public Map<String, AiToolMetric> metrics(Connection c) throws Exception {
        Map<String, long[]> values = new HashMap<String, long[]>();
        PreparedStatement totals = c.prepareStatement(
                "SELECT tool_name,COUNT(*),SUM(status='SUCCEEDED'),SUM(status='FAILED'),"
                        + "COALESCE(AVG(CASE WHEN completed_at IS NOT NULL THEN "
                        + "TIMESTAMPDIFF(MICROSECOND,created_at,completed_at)/1000 END),0),"
                        + "MAX(created_at) FROM ai_tool_call_logs GROUP BY tool_name");
        try {
            ResultSet rs = totals.executeQuery();
            try {
                while (rs.next()) values.put(rs.getString(1), new long[]{rs.getLong(2),
                        rs.getLong(3), rs.getLong(4), Math.round(rs.getDouble(5)),
                        rs.getTimestamp(6).getTime()});
            } finally { rs.close(); }
        } finally { totals.close(); }
        Map<String, String> errors = new HashMap<String, String>();
        PreparedStatement recent = c.prepareStatement(
                "SELECT l.tool_name,l.result_summary FROM ai_tool_call_logs l JOIN "
                        + "(SELECT tool_name,MAX(id) AS id FROM ai_tool_call_logs "
                        + "WHERE status='FAILED' GROUP BY tool_name) x ON x.id=l.id");
        try {
            ResultSet rs = recent.executeQuery();
            try { while (rs.next()) errors.put(rs.getString(1), rs.getString(2)); }
            finally { rs.close(); }
        } finally { recent.close(); }
        Map<String, AiToolMetric> out = new HashMap<String, AiToolMetric>();
        for (Map.Entry<String, long[]> entry : values.entrySet()) {
            long[] value = entry.getValue();
            out.put(entry.getKey(), new AiToolMetric(value[0], value[1], value[2], value[3],
                    value[4], errors.get(entry.getKey())));
        }
        return out;
    }
}
