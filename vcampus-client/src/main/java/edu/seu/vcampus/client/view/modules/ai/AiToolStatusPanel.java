...
public final class AiToolStatusPanel extends SectionCard {
    ...
    private final DefaultTableModel model = new DefaultTableModel(
            new Object[]{"状态", "类型", "工具", "能力", "调用", "成功率", "平均耗时", "最近调用", "最近错误"}, 0) {
    ...
    private JComponent content() {
        ...
        JScrollPane scroll = new JScrollPane(table); scroll.setPreferredSize(new Dimension(980, 460));
                ...
                try {
                    ...
                    for (AiToolStatus row : get()) {
                        ...
                        model.addRow(new Object[]{row.isAvailable() ? "可用" : "未接入",
                                ...
                                row.getName(), row.getDescription(), Long.valueOf(row.getCalls()),
                                row.getCalls() == 0 ? "—" : String.format("%.1f%%",
                                        row.getSucceeded() * 100.0 / row.getCalls()),
                                row.getCalls() == 0 ? "—" : row.getAverageDurationMillis() + " ms",
                                row.getLastCalledAt() == 0 ? "—" : new java.util.Date(row.getLastCalledAt()),
                                row.getLastError() == null ? "" : row.getLastError()});
                    }
