...
public final class NetworkPdfClientService implements PdfClientService {
    ...
    private Object request(String command, java.io.Serializable body) throws Exception {
        String current = session == null ? network.getSessionToken() : currentToken();
        if (current == null || current.trim().isEmpty()) throw new IllegalStateException("请先登录");
        return network.requestWithToken(command, body, current).getPayload();
    }
    private String currentToken() {
        if (session == null || !session.isAuthenticated()) throw new IllegalStateException("登录会话已结束，请重新登录后操作");
