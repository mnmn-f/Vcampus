...
final class LibraryUi {
    ...
    static JTextArea paragraph(String text, int rows) {
        JTextArea a = new JTextArea(text, rows, 1); a.setLineWrap(true); a.setWrapStyleWord(true);
        a.setEditable(false); a.setFocusable(false); a.setOpaque(false); a.setBorder(null);
        a.setFont(DesignTokens.regular(13)); a.setForeground(MUTED); return a;
    }
    ...
    static String html(String value) { return plain(value).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;").replace("\"", "&quot;"); }
