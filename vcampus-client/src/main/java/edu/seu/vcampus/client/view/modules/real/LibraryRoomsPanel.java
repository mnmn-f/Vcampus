...
public final class LibraryRoomsPanel extends JPanel {
    ...
    public LibraryRoomsPanel(BasePage page, LibraryClientService service, Role role) {
        ...
        else { editor = new LibraryRoomEditorPanel(new LibraryRoomEditorPanel.Listener() {
            @Override public void onSave(StudyRoomUpsertRequest request) { LibraryRoomsPanel.this.saveRoom(request); }
        }); add(editor); reservation = null; reservations = null; }
    }

    private AsyncPagedTable<StudyRoomView> roomTable() {
        AsyncPagedTable<StudyRoomView> table = new AsyncPagedTable<StudyRoomView>("自习室", "开放状态、容量和开放时段。", "搜索楼栋或房间号", new String[]{"全部状态", "开放", "维护中", "关闭"}, new String[]{"编号", "楼栋", "房间号", "容量", "开放时段", "状态"},
        ...
        if (role == Role.STUDENT) { JButton select = new PrimaryButton("预约选中房间"); select.addActionListener(new java.awt.event.ActionListener() {
            @Override public void actionPerformed(java.awt.event.ActionEvent e) { chooseRoom(); }
        }); table.addAction(select); }
        if (role == Role.LIBRARIAN) { JButton create = new PrimaryButton("新建自习室"); create.addActionListener(new java.awt.event.ActionListener() {
