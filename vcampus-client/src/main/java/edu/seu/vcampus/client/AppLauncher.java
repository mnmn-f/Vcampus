...
import edu.seu.vcampus.client.auth.NetworkAiAssistantClientService;
...
public final class AppLauncher {
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(new Runnable() {
            @Override public void run() {
                ...
                showLogin(runtime.auth, session, runtime.business, runtime.assistant);
            }
        });
    }
    ...
    private static RuntimeServices createRuntime(ClientSession session) {
        ...
        try {
            ...
            int aiReadTimeout = readPositiveInt("vcampus.ai.client-read-timeout", 120000);
            AiAssistantClientService assistant = new NetworkAiAssistantClientService(
                    network, session, host, port, connectTimeout, aiReadTimeout);
            return new RuntimeServices(new NetworkAuthClientService(gateway),
                    new ClientBusinessServices(network, session), assistant);
    ...
    private static void showLogin(final AuthClientService auth, final ClientSession session,
                                  ...
                                  final AiAssistantClientService assistant) {
    ...
    private static final class RuntimeServices {
        ...
        private final AiAssistantClientService assistant;

        private RuntimeServices(AuthClientService auth, ClientBusinessServices business,
                                AiAssistantClientService assistant) {
            this.auth = auth;
