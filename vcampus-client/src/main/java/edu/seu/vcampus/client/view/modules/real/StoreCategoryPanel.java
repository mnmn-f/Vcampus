...
public final class StoreCategoryPanel extends SectionCard {
    ...
    private final JComboBox<RealUi.CodeOption> active = new JComboBox<RealUi.CodeOption>(RealUi.options("ACTIVE", "INACTIVE"));
    ...
    public StoreCategoryPanel(BasePage page, StoreClientService service) {
        ...
        table = new AsyncPagedTable<StoreCategoryDto>("分类列表", "", "分类关键字", new String[0], new String[]{"分类编码", "名称", "状态"}, new AsyncPagedTable.Loader<StoreCategoryDto>() {
            @Override public PageSlice<StoreCategoryDto> load(int p, String k, String f) throws Exception { StoreCategoryPage v=StoreCategoryPanel.this.service.listCategories(); return PageSlice.filter(v.getItems(), k, p, 20, row -> row.getCode() + " " + row.getName()); }
        }, new AsyncPagedTable.RowMapper<StoreCategoryDto>() { @Override public Object[] values(StoreCategoryDto v){return new Object[]{RealUi.text(v.getCode()),RealUi.text(v.getName()),v.isActive()?"启用":"停用"};} }, new AsyncPagedTable.SelectionListener<StoreCategoryDto>() { @Override public void onSelected(StoreCategoryDto v){ if(v==null)return;selectedId=v.getId();code.setText(v.getCode());code.setEditable(false);name.setText(v.getName());active.setSelectedItem(RealUi.option(v.isActive()?"ACTIVE":"INACTIVE")); } });
        table.setItemKey(StoreCategoryDto::getId);
